...
qcow2, one builder run each -- the qcow2 through `image-builder build
--bootc-ref`, the ISO through the compatibility entry point, which is the only
one that still builds it), and the GitHub Release that points at all of them.
The ISO build takes 20 to 35 minutes, so a release takes roughly an

...
- Three machines on one broadcast domain, each booted from the Corium image
  ...
  three cloud instances, or three physical machines. [Proxmox](/docs/install/proxmox/) and
  [OpenStack](/docs/install/openstack/) each walk through creating one; the script at the
...
- The Corium qcow2 or ISO for whichever platform you use — see
  [downloads](/docs/install/downloads/).
...
[Proxmox step 3](/docs/install/proxmox/#3-describe-the-node) — it is the same one, and
without it the machine boots with no account to SSH into.
[`examples/ha-controller-first.yaml`](https://github.com/Corium-OS/Corium/blob/main/docs/examples/ha-controller-first.yaml) is
...
[`examples/ha-controller-join.yaml`](https://github.com/Corium-OS/Corium/blob/main/docs/examples/ha-controller-join.yaml) shows
the whole document.
...
the host the same way [Proxmox](/docs/install/proxmox/#before-you-start) does, and run it
from `deploy/proxmox/`.
...
Two things about an HA cluster are easier with the [management API](/docs/reference/cli/)
...
- [cctl](/docs/reference/cli/) — every command, the three roles, and what the API will not
  do
- [Configuration](/docs/reference/configuration/) — every field of `ha:` and `join:`

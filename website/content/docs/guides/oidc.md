---
title: "OIDC login"
description: "Wiring an OpenID Connect provider to the cluster's API server through k0s.patch, a kubeconfig that logs in with kubelogin, and RBAC by group."
slug: "oidc"
draft: false
weight: 127
toc: true
...
<!-- Generated by website/sync-docs.py from docs/oidc.md. Edit that file, not this one. -->

Corium does not model the kube-apiserver's flags, so authentication is one of
the things it leaves to the [`k0s.patch`](/docs/reference/configuration/#315-k0spatch--the-escape-hatch)
...
restarts the control plane. See [ADR 8](/docs/decisions/adr-0008-day-two-reconcile/).

---
title: "ZFS on data disks"
description: "Declaring ZFS pools on a node's data disks with the ZFS image variant, why the module is baked against the image's kernel, and how it differs from software RAID."
slug: "zfs"
draft: false
weight: 131
toc: true
...
<!-- Generated by website/sync-docs.py from docs/zfs.md. Edit that file, not this one. -->
...
- **It needs the ZFS image.** ZFS is a kernel module Fedora does not ship, so it
  ...
  [`deploy/zfs/`](https://github.com/Corium-OS/Corium/blob/main/deploy/zfs/README.md) and run your nodes on that.
- **It does not cover the root disk.** Like RAID, this is data disks only. Root
  ...
  [ADR-0007](/docs/decisions/adr-0007-zfs-data-disks/).
...
[deploy/zfs/README.md](https://github.com/Corium-OS/Corium/blob/main/deploy/zfs/README.md). The rest of this page assumes a
...
Every field is in the [configuration reference](/docs/reference/configuration/#316-zfs).
...
(see [software RAID](/docs/guides/raid/#why-this-is-not-just-cloud-init)):
...
- [Configuration reference §3.17](/docs/reference/configuration/#317-zfs)
- [Building the ZFS image](https://github.com/Corium-OS/Corium/blob/main/deploy/zfs/README.md)
- [ADR-0007: ZFS covers data disks](/docs/decisions/adr-0007-zfs-data-disks/)
- [Software RAID](/docs/guides/raid/) — the other storage feature, and when to prefer it

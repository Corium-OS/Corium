...
/**
 * @file
 * Project override of the Doks FlexSearch script.
 *
 * The only change from the upstream theme file is the fetch URL in
 * buildIndex(): the theme requests "{{ site.LanguagePrefix }}/search-index.json",
 * which resolves to a host-absolute "/search-index.json" for the default,
 * non-subdir language. GitHub Pages serves this site under the /Corium/ subpath,
 * so that request 404s, the index never loads, and every query returns
 * "No results". Piping the path through relLangURL joins it to the full baseURL
 * (path included) and keeps the language prefix when one applies, so the fetch
 * hits /Corium/search-index.json. Same class of bug the config comments guard
 * against with absURL/relURL for the "Get started" button.
 */

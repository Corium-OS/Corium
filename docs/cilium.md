...
Corium ships kube-router because k0s does: one component covering networking,
network policy and service proxying. Replacing it with Cilium takes two
decisions, and both fit in the cloud-config the node already boots with.

First, k0s must be told to install nothing, so that two CNIs do not fight over
the same node. Second, something has to install Cilium. `cni: custom` handles
the first; an entry under `addons:` handles the second, and the node still
converges on its own without a second provisioning step.
...
The node goes `Ready` within a minute or so of the agent reporting itself
healthy. `cilium status --wait`, if you install the
...
[configuration reference](reference.md#33-network), and what else `k0s.patch`

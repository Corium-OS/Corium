...
with it. This page tracks `main`. The current stable release is **0.3.6**, and a
...
complete working document? See [examples](examples.md). Want to be walked
through it? Start with the [quick start](quickstart.md). For what Corium models
versus what it passes through to k0s, see [feature support](features.md).
...
before claiming such a node — see [cctl](cli.md#claiming-a-node-that-already-knows-what-it-is).
...
other. See [ADR 6](adr/0006-host-wireguard-overlay.md).
...
enabled `wg-quick@` unit — and [ADR 6](adr/0006-host-wireguard-overlay.md) covers
...
> [downloads](install/downloads.md#installing-cctl). The daemon runs unconfined
> under SELinux; [ADR 4](adr/0004-management-api.md) records why and what a
...
Claiming a node, and everything afterwards, is [`cctl`](cli.md). One thing to
...
`k0s reset` rather than reasoned about. [ADR 4](adr/0004-management-api.md)
...
[disk encryption](luks.md) for the guide and
[ADR 10](adr/0010-luks-data-disks.md) for what it does and does not protect
...
> [Enrol one by hand](luks.md#adding-a-recovery-key).

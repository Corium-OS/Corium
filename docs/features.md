...
| Version ladder | n/a | `0.2.0`, `0.2` and `latest` per release. No major rung below 1.0, because a 0.x minor may break. See [upgrades](upgrades.md#choosing-what-to-track) |
...
| Root filesystem on RAID | n/a | Works, but install-time only: hand-written Kickstart, a second ESP, and an fstab edit. See [software RAID](raid.md) |

### Overlay networking

| Feature | Field | Notes |
|---|---|---|
| Host WireGuard interface | `wireguard[]` | Brought up before k0s so a cluster can run over an encrypted overlay between hosts. See [ADR 6](adr/0006-host-wireguard-overlay.md) |
...
| **Removing add-ons** | k0s's Helm extensions install charts; Corium does not model uninstalling one. Remove a release with `kubectl delete chart <name> -n kube-system`, per [k0s: Helm charts](https://docs.k0sproject.io/stable/helm-charts/) |

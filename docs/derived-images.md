...
```dockerfile
FROM ghcr.io/corium-os/corium:0.3.0
...
```bash
podman build --tag ghcr.io/you/corium:0.3.0-1 --file Containerfile .
podman push ghcr.io/you/corium:0.3.0-1
```
...
cosign sign --key cosign.key ghcr.io/you/corium:0.3.0-1
```
...
```bash
cctl upgrade node-a node-b --image ghcr.io/you/corium:0.3.0-1
```
...
Corium publishes. Pin the version you tested — `0.3.0`, not `latest` — and move
it deliberately.

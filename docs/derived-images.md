...
```bash
podman build --tag ghcr.io/you/corium:0.3.6-1 --file Containerfile .
podman push ghcr.io/you/corium:0.3.6-1
```
...
cosign sign --key cosign.key ghcr.io/you/corium:0.3.6-1
```
...
```bash
cctl upgrade node-a node-b --image ghcr.io/you/corium:0.3.6-1
```
...
Corium publishes. Pin the version you tested — `0.3.6`, not `latest` — and move
it deliberately.

...
oras pull ghcr.io/corium-os/corium-iso:0.3.0
...
curl -L -H "Authorization: Bearer ${token}" -o corium-0.3.0-x86_64.iso \
...
https://<cdn>/corium-0.3.0-x86_64.iso
```
...
`mise use 'github:Corium-OS/Corium[exe=cctl]@0.3.0'` — read release assets and
not registries.
...
```bash
sha256sum corium-0.3.0-x86_64.iso

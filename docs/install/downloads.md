...
oras pull ghcr.io/corium-os/corium-iso:0.3.3
...
curl -L -H "Authorization: Bearer ${token}" -o corium-0.3.3-x86_64.iso \
...
https://<cdn>/corium-0.3.3-x86_64.iso
```
...
`mise use 'github:Corium-OS/Corium[exe=cctl]@0.3.3'` — read release assets and
not registries.
...
```bash
sha256sum corium-0.3.3-x86_64.iso
...
the filesystem it creates. See the [quick start](../quickstart.md).

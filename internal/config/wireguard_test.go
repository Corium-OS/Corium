...
func TestValidateWireGuard(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:    "missing name",
			wg:      []WireGuardInterface{{Address: "10.10.0.2/24", PrivateKey: priv}},
			wantErr: "wireguard[0].name: required",
		},
		{
			name:    "invalid interface name",
			wg:      []WireGuardInterface{{Name: "wg 0", Address: "10.10.0.2/24", PrivateKey: priv}},
			wantErr: "must be lowercase",
		},
		{
			name:    "interface name too long",
			wg:      []WireGuardInterface{{Name: "wglonginterface0", Address: "10.10.0.2/24", PrivateKey: priv}},
			wantErr: "the limit is 15",
		},
		{
			name: "duplicate interface names",
			wg: []WireGuardInterface{
				{Name: "wg0", Address: "10.10.0.2/24", PrivateKey: priv},
				{Name: "wg0", Address: "10.10.1.2/24", PrivateKey: priv},
			},
		...
		},
		{
			...
			wg:      []WireGuardInterface{{Name: "wg0", Address: "10.10.0.2", PrivateKey: priv}},
			wantErr: "must be an address with a prefix length",
		},
		{
			name:    "listen port out of range",
			wg:      []WireGuardInterface{{Name: "wg0", Address: "10.10.0.2/24", ListenPort: 70000, PrivateKey: priv}},
			wantErr: "listenPort: 70000 is out of range",
		},
		{
			name:    "mtu out of range",
			wg:      []WireGuardInterface{{Name: "wg0", Address: "10.10.0.2/24", MTU: 100, PrivateKey: priv}},
			wantErr: "mtu: 100 is out of range",
		},
		{
			name:    "no private key",
			wg:      []WireGuardInterface{{Name: "wg0", Address: "10.10.0.2/24"}},
		...
		},
		{
			...
			wg:      []WireGuardInterface{{Name: "wg0", Address: "10.10.0.2/24", PrivateKey: "not base64!!"}},
			wantErr: "must be a base64-encoded key",
		},
		{
			name: "two node addresses",
			wg: []WireGuardInterface{
				{Name: "wg0", Address: "10.10.0.2/24", NodeAddress: true, PrivateKey: priv},
				{Name: "wg1", Address: "10.20.0.2/24", NodeAddress: true, PrivateKey: priv},
			},
			wantErr: "more than one interface sets nodeAddress",
		},
		{
			name: "peer without a public key",
			wg: []WireGuardInterface{{Name: "wg0", Address: "10.10.0.2/24", PrivateKey: priv,
...
func TestWireGuardNodeAddress(t *testing.T) {
	cfg := &Config{WireGuard: []WireGuardInterface{
		{Name: "wg0", Address: "10.10.0.5/24"},
		{Name: "wg1", Address: "10.20.0.5/16", NodeAddress: true},
	}}
	...
	none := &Config{WireGuard: []WireGuardInterface{{Name: "wg0", Address: "10.10.0.5/24"}}}
